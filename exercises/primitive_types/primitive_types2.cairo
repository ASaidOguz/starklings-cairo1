...
fn main() {
    ...
    let is_alphabetic= true;// Finish this line like the example! What's your favorite short string?
    let mut your_character='M';// Try a letter, try a number, try a special character, try a short string!
    if is_alphabetic(
        ref your_character
    ) {
        ('Alphabetical!').print();
