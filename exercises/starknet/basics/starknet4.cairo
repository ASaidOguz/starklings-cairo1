...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState,product:felt252,new_stock:usize ) {
            // TODO:
            // * takes product and new_stock
            // * adds new_stock to stock in inventory
            // * only owner can call this
            assert(get_caller_address()==self.contract_owner.read(),'Unautharized-caller');
            let old_stock=self.products.read(product);
            let final_stock=new_stock+old_stock;
            self.products.write(product,final_stock);
        }
        ...
        fn purchase(ref self: ContractState,product:felt252,quantity:u32) {
            ...
            let old_quantity=self.products.read(product);
            let new_quantity=old_quantity-quantity;
            self.products.write(product,new_quantity);
        }
